...
- (void)test_receiver {
    ...
    holder.receiver().receivable().receive_value(map::make_fetched_event(std::map<int, std::string>{{2, "12"}}));

    XCTAssertEqual(holder.size(), 1);
    XCTAssertEqual(holder.at(2), "12");

    holder.receiver().receivable().receive_value(map::make_any_event(std::map<int, std::string>{{3, "13"}, {4, "14"}}));

    XCTAssertEqual(holder.size(), 2);
    XCTAssertEqual(holder.at(3), "13");
    XCTAssertEqual(holder.at(4), "14");

    holder.receiver().receivable().receive_value(
        map::make_inserted_event(std::map<int, std::string>{{5, "15"}, {6, "16"}}));
    ...
    holder.receiver().receivable().receive_value(
        map::make_erased_event(std::map<int, std::string>{{4, "14"}, {5, "15"}}));

    XCTAssertEqual(holder.size(), 2);
    XCTAssertEqual(holder.at(3), "13");
    XCTAssertEqual(holder.at(6), "16");

    holder.receiver().receivable().receive_value(map::make_replaced_event<int, std::string>(3, "23"));
